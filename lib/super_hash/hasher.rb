...
module SuperHash
  ...
  class Hasher < ::Hash
      ...
      if init_value.respond_to?(:each_pair)
        ...
        init_value.each do |att, value|
          self.[]=(
            att,
            value,
            skip_validate_attribute: true,
            after_set_callbacks: set_callbacks
          )
        end
    ...
    def validate_all_attributes!
      klass_attributes.each do |name, _|
    ...
    def set_defaults
      ...
      klass_attributes.each do |name, attr_options|
        ...
        unless default_value.nil? && !attr_required?(name) && ignore_nil_default_values
